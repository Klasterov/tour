...
const sections = ['section-route-map','section-about','section-info','section-departure','section-similar','section-reviews'];
const tabMap = {
  'section-route-map': 'section-route',
  'section-about': 'section-about',
  'section-info': 'section-info',
  'section-departure': 'section-departure',
  'section-similar': 'section-similar',
...
window.addEventListener('scroll', () => {
  ...
  tabs.forEach(tab => {
    ...
    if (href === tabMap[current] || href === current) tab.classList.add('active');
  });
});
})();
...
btn.addEventListener('click', () => {
  ...
  btn.textContent = open ? 'Скрыть ▴' : 'Читать полностью ▾';
});
})();

// ── SIMILAR CARDS SLIDER ──
(function() {
const slider = document.getElementById('similarSlider');
const prev   = document.getElementById('simPrev');
const next   = document.getElementById('simNext');
if (!slider || !next) return;

let idx = 0;
const cards = slider.querySelectorAll('.sim-card');
const visible = 4;
const maxIdx = Math.max(0, cards.length - visible);

function update() {
  const w = cards[0] ? cards[0].offsetWidth + 16 : 0;
